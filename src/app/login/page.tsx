...
import { loginUser, signInWithGoogle, getAuthErrorMessage } from '@/lib/auth';
import { useAuth } from '@/contexts/AuthContext';
import { useT } from '@/contexts/LangContext';
import toast from 'react-hot-toast';
import { FiTerminal, FiMail, FiLock, FiEye, FiEyeOff, FiLogIn, FiLoader } from 'react-icons/fi';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (err?.code?.includes('email') || err?.code?.includes('user-not-found')) {
        setEmailError(msg);
      }
    } finally {
      setLoading(false);
      ...
      <div className="w-full max-w-md">
        <div className="terminal-window mb-8 text-center">
          <div className="terminal-window-header justify-center">
            ...
            <span className="text-text-muted text-xs font-mono mr-auto ml-auto">{'>'} auth/login.sh</span>
          </div>
          <div className="p-6">
            <FiTerminal className="text-primary text-4xl mx-auto mb-3" />
            <h1 className="text-2xl font-bold text-text font-mono">{t('auth.login.title')}</h1>
            <p className="text-text-muted text-sm mt-1 font-mono">{t('auth.login.subtitle')}</p>
          </div>
        </div>
        ...
        <div className="terminal-window">
          <div className="terminal-window-header">
            ...
            <span className="terminal-dot terminal-dot-yellow" />
            <span className="terminal-dot terminal-dot-green" />
            <span className="text-text-muted text-xs font-mono mr-auto">credentials.sh</span>
          </div>
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            <div>
              ...
              {emailError && (
                <p className="text-red-500 text-xs mt-1 font-mono">✗ {emailError}</p>
              )}
            </div>
            ...
            <div className="flex items-center justify-between text-xs">
              <Link href="/forgot-password" className="text-text-muted hover:text-primary transition-colors font-mono">
                {t('auth.forgotLink')}
              </Link>
            </div>
